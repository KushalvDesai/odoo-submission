...
const NotificationDropdown = ({ open }: { open: boolean }) => {
  ...
  return (
    <div className="absolute right-0 mt-2 w-80 bg-[#23272a] rounded-xl shadow-lg z-50 border border-[#40444b]">
      <div className="p-4 border-b border-[#40444b] text-white font-semibold">Notifications</div>
      <ul className="max-h-80 overflow-y-auto">
        {notifications.length === 0 && (
          <li className="p-4 text-[#b5bac1] text-sm">No notifications</li>
        )}
        {notifications.slice(0, 10).map(n => (
          <li key={n.id} className={`px-4 py-3 border-b border-[#40444b] ${n.isRead ? "bg-transparent" : "bg-[#313338]"}`}>
            <div className="text-white text-sm">{n.message}</div>
            <div className="text-xs text-[#b5bac1] mt-1">{n.createdAt.toLocaleString()}</div>
          </li>
        ))}
      </ul>
    </div>
